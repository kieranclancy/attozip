...
static void zip_path(int zip, int dirfd, const char *path);
static void zip_file(int zip, char *path, int fd, struct stat *st);
static void zip_build_directory(int zip);
static void path_add(char *buf, int bufsz, const char *add);
static uint32_t msdos_time(struct tm *t);
static uint16_t zip_perms(mode_t mode);
static uint32_t crc32_add(uint32_t crc32, const uint8_t *buf, size_t n);
static void write_32_le(int fd, uint32_t x);
static void write_16_le(int fd, uint16_t x);
static void xerr(const char *str);
static void xerr2(const char *a, const char *b);

typedef struct zipindex {
    char *fname;
    int namelen;
    off_t offset;
    uint32_t time;
    ...
    uint32_t size;
    uint16_t perms;
    struct zipindex *next;
} zipindex_t;

static zipindex_t zindex = { 0 };
...
int main(int argc, char *argv[]) {
    ...
    for (i=2; i<argc; i++) zip_path(zip, AT_FDCWD, argv[i]);

    zip_build_directory(zip);

    return 0;
}

static void zip_path(int zip, int dirfd, const char *path) {
    static char zpath[PBUF] = "";
    struct stat st;
    struct dirent *dent;
    char *fname;
    int in, zpathlen;
    ...
    if (!path || !*path) xerr("invalid path.");

    islink = 0;
    readlinkat(dirfd, path, &islink, 1);
    if (islink) xerr("symlinks not supported.");

    in = openat(dirfd, path, O_RDONLY);

    zpathlen = strlen(zpath);
    path_add(zpath, PBUF, path);
    if (in < 0 || fstat(in, &st)) {
        xerr2(zpath, "could not open");
    }
    ...
    if (!S_ISDIR(st.st_mode)) xerr2(zpath, "unexpected file type");

    d = fdopendir(in);
    if (!d) xerr2(zpath, "could not open");

    while ((dent = readdir(d))) {
        fname = dent->d_name;
        if (!fname || !*fname) xerr2(zpath, "has bad file entry");
...
#define zwrite(s,n) do { \
    if (write(zip, s, n) < 0) xerr("write error"); \
} while (0)
#define zw32le(x) write_32_le(zip, x)
#define zw16le(x) write_16_le(zip, x)
#define zseek(o,w) do { \
    if (lseek(zip, o, w) < 0) xerr("seek error"); \
} while (0)
static void zip_file(int zip, char *path, int fd, struct stat *st) {
    static uint8_t buf[RBUF];
    zipindex_t *indp = &zindex;
    int rbytes;

    if (!path || !*path) xerr("bad path");
    while (indp->fname) {
        if (strcmp(indp->fname, path) == 0) xerr2(path, "duplicate path");
        indp = indp->next;
    }

    indp->fname = strdup(path);
    indp->next = calloc(1, sizeof(*indp));
    if (!indp->fname || !indp->next) xerr("out of memory");

    indp->namelen = strlen(path);
    indp->offset = lseek(zip, 0, SEEK_CUR);
    indp->time = msdos_time(localtime(&(st->st_mtime)));
    indp->perms = zip_perms(st->st_mode);

    zwrite("\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00", 10);
    zw32le(indp->time);
    zwrite("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12);
    zw16le(indp->namelen);
    zwrite("\x00\x00", 2);
    zwrite(indp->fname, indp->namelen);

    indp->size = 0;
    indp->crc32 = 0;
    do {
        rbytes = read(fd, buf, RBUF);
        if (rbytes == 0) break;
        if (rbytes < 0) xerr2(path, "read error");
        zwrite(buf, rbytes);
        indp->size += rbytes;
        indp->crc32 = crc32_add(indp->crc32, buf, rbytes);
    } while (1);
    zseek(indp->offset + 14, SEEK_SET);
    zw32le(indp->crc32);
    zw32le(indp->size);
    zw32le(indp->size);
    zseek(4 + indp->namelen + indp->size, SEEK_CUR);
}

static void zip_build_directory(int zip) {
    uint16_t files = 0;
    uint32_t d_offset, eod_offset;
    zipindex_t *indp = &zindex;
    ...
    while (indp->fname) {
        ...
        zw32le(indp->size);
        zw16le(indp->namelen);
        zwrite("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 10);
        zw16le(indp->perms);
        zw32le(indp->offset);
        zwrite(indp->fname, indp->namelen);
        files++;
    ...
    eod_offset = lseek(zip, 0, SEEK_END);
    zwrite("\x50\x4b\x05\x06\x00\x00\x00\x00", 8);
    zw16le(files);
    zw16le(files);
    zw32le(eod_offset-d_offset);
    zw32le(d_offset);
    zwrite("\x00\x00", 2);
}
...
static void path_add(char *buf, int bufsz, const char *add) {
    ...
    while (*add) {
        ...
        if (len == 0 || (len == 1 && add[0] == '.')) goto next_component;
        
        if (len == 2 && add[0] == '.' && add[1] == '.') {
            if (exdepth-- == 0) xerr("unexpected /../ in path");
            if (!(p = strrchr(buf, '/'))) p = buf;
            *p = '\0';
            goto next_component;
        }

        if (strlen(buf) + 1 + len + 1 > bufsz) xerr("path too long");
        if (buf[0]) strcat(buf, "/");
        strncat(buf, add, len);
        if (exdepth >= 0) exdepth++;
...
static uint32_t msdos_time(struct tm *t) {
    if (!t) xerr("time not set");
    /* needs to be written as little-endian */
    return (((( /* 7bit */ (t->tm_year-80)
                << 4     ^ (t->tm_mon+1  ))
                << 5     ^ (t->tm_mday   ))
                << 5     ^ (t->tm_hour   ))
                << 6     ^ (t->tm_min    ))
...
static uint16_t zip_perms(mode_t mode) {
    if ((mode & S_IFMT) != S_IFREG) xerr("zipping non-file?");
    return mode & (S_IFMT | 07777);
}

static uint32_t crc32_add(uint32_t crc32, const uint8_t *buf, size_t n) {
    /* divisions by polynomial 0x[1]04c11db7 for little-endian output */
    /* CRC32 algorithm and table generation is publicly documented    */
    static const uint32_t c32t[256] = { 0x00000000, 0x77073096, 0xee0e612c,
